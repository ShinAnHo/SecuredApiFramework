    ...
    public class OauthManager : BaseBusinessLogic, IOauthManager
    {
        ...
        public async Task<string> Authorize(AuthorizationRequest data)
        {
            ...
            else
            {
                ...
                using (_uow)
                {
                    var clientRepository = new ClientRepository(_uow);

                    try
                    {
                        var currentData = await clientRepository.ReadByLambda(u => u.Name == data.client_id && u.IsActive == true);
        ...
        public async Task<AuthorizationResponse> Authorize(AuthenticateRequest data, string state)
        {
            ...
            using (_uow)
            {
                ...
                var authorizationCodeRepository = new AuthorizationCodeRepository(_uow);

                try
                {
                    var currentData = await userRepository.ReadByLambda(u => u.UserName == data.UserName && u.IsActive == true);
                    ...
                    else
                    {
                        ...
                        {
                            var AuthCode = await authorizationCodeRepository.GenerateCode(currentUser.UserId, _appSettings.CodeTokenExpiration);
        ...
        public async Task<AccessTokenResponse> Token(AccessTokenRequest data)
        {
            ...
            else if (data.grant_type == "client_credentials")
            {
                ...
                using (_uow)
                {
                    ...
                    ClientIdentityClaimRepository clientIdentityClaimRepository = new ClientIdentityClaimRepository(_uow);
                    ClientSecretRepository clientSecretRepository = new ClientSecretRepository(_uow);

                    try
                    {
                        var clients = await clientRepository.ReadByLambda(u => u.Name == data.client_id && u.IsActive == true);

                        if (!clients.Any())
                            throw new OauthException(OauthErrorCode.invalid_client);

                        var currentClient = clients.First();
                        var clientSecrets = await clientSecretRepository.GetSecret(currentClient.ClientId);

                        if (clientSecrets != data.client_secret)
                            throw new OauthException(OauthErrorCode.invalid_client);
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (currentRefreshToken.Revoked)
                                throw new OauthException(OauthErrorCode.invalid_token, "Token revoked");
                            else if (currentRefreshToken.ExpiryDate < DateTime.Now)
                                throw new OauthException(OauthErrorCode.invalid_token, "Token expired");
                            else
                            {
        ...
        public async Task Revoke(RevokeTokenRequest data)
        {
            if (data.client_id == null || data.client_secret == null)
                throw new OauthException(OauthErrorCode.access_denied);
            else if (data.token_type_hint == "refresh_token")
            {
                _uow.OpenConnection(ConnectionString.DefaultConnection);
                ...
                {
                    ClientRepository clientRepository = new ClientRepository(_uow);
                    ClientScopeRepository clientScopeRepository = new ClientScopeRepository(_uow);
                    ClientSecretRepository clientSecretRepository = new ClientSecretRepository(_uow);
                    RefreshTokenRepository refreshTokenRepository = new RefreshTokenRepository(_uow);

                    try
                    {
                        var clients = await clientRepository.ReadByLambda(c => c.Name == data.client_id && c.IsActive == true);
                        ...
                        var isAuthorized = await clientScopeRepository.CheckScope(currentClient.ClientId, "identityserver.write");

                        if (!isAuthorized)
                            throw new OauthException(OauthErrorCode.invalid_client, "Not authorized client");

                        var refreshTokens = await refreshTokenRepository.ReadByLambda(rt => rt.Token == data.token);

                        if (!refreshTokens.Any())
                            throw new OauthException(OauthErrorCode.invalid_token);

                        var currentToken = refreshTokens.First();
                        if (currentToken.Revoked == false)
                        {
                            currentToken.Revoked = true;
                            await refreshTokenRepository.Update(currentToken);
                        }
                    }
                    catch (Exception)
                    {
