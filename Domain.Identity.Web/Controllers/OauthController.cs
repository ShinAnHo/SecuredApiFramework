...
namespace Domain.Identity.Web
{
    ...
    public class OauthController : Controller
    {
        ...
        public async Task<IActionResult> Authorize(string response_type, string client_id, string redirect_uri, string scope, string state)
        {
            try
            {
                var requestData = new AuthorizationRequest()
                {
                    ...
                    client_id = client_id,
                    redirect_uri = NullHandler.ConvertString(redirect_uri),
                    scope = NullHandler.ConvertString(scope),
                    state = NullHandler.ConvertString(state)
                };
        ...
        public async Task<IActionResult> Revoke(string token, string token_type_hint)
        {
            try
            {
                if (token_type_hint == "refresh_token") { 
                
                    var requestData = new RevokeTokenRequest()
                    {
                        token = token,
                ...
                await _oauthCodeManager.Revoke(requestData);
